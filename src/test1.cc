...
/*
Again, the decryption operation is much the same as for normal symmetric decryption as described
here. The main differences are:

You may optionally pass through an IV length using EVP_CIPHER_CTX_ctrl
AAD data is passed through in zero or more calls to EVP_DecryptUpdate, with the output buffer set to
NULL Prior to the EVP_DecryptFinal_ex call a new call to EVP_CIPHER_CTX_ctrl provides the tag A non
positive return value from EVP_DecryptFinal_ex should be considered as a failure to authenticate
ciphertext and/or AAD. It does not necessarily indicate a more serious error.
