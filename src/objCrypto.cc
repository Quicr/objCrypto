...
ObjCryptor::seal(KeyID keyID, const std::variant<Nonce, IV> &nonceOrIV,
                 const std::vector<uint8_t> &plainText, std::vector<uint8_t> &cipherText) const {
    assert(haveKey(keyID));
...
__attribute__((visibility("default"))) ObjCryptoErr
ObjCryptor::unseal(KeyID keyID, const std::variant<Nonce, IV> &nonceOrIV,
                   const std::vector<uint8_t> &cipherText, std::vector<uint8_t> &plainText) const {
    assert(haveKey(keyID));
    const KeyInfo &keyInfo = keyInfoMap.at(keyID);
